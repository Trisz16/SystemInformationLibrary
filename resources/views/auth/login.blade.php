<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <h2 class="text-2xl font-bold text-center text-indigo-700 mb-6">Masuk Anggota</h2>
        ...
        <div class="flex items-center justify-end">
            <!-- Menambahkan link Register di sini agar mudah diakses -->
             @if (Route::has('register'))
                <a class="text-sm text-indigo-600 hover:text-indigo-900 font-medium mr-4" href="{{ route('register') }}">
                    Belum punya akun? Daftar
                </a>
            @endif

            <x-primary-button class="ms-3 bg-indigo-600 hover:bg-indigo-700 transition duration-150">
                {{ __('Masuk') }}
            </x-primary-button>
        </div>
    </form>
